...
library(dplyr)
library(ggplot2)
ber_design <- read.csv("output/design_labeled_bernoulli_0.95.csv", stringsAsFactors = FALSE)
pois_design <- read.csv("output/design_labeled_poisson_0.95.csv", stringsAsFactors = FALSE)
  ...
  if(case_id %in% c(1,2)){
    case_df = ber_design %>% 
      ...
      select(effect,
             ...
             bayes_treat = bayes_treat_0.95,
             bayes_treat_peek = bayes_treat_peek_0.95,
             peek_multiplier_bayes)
    if(case_id == 1){
      case_df = case_df %>% 
        filter(effect == 0) 
    } else {
