...
for(row in 1:nrow(ber_design)){
  ...
  per_day <- ber_design[row, 'sample_size_per_day']
  thresholds <- c(0.95)
  ...
  ret <- ret %>%
    mutate(prob = do.call(vec_prob_bernoulli, .)) %>%
...
write.csv(ber_design, "output/design_bernoulli_0.95.csv", row.names = FALSE)
...
for(row in 1:nrow(pois_design)){
  ...
  per_day <- pois_design[row, 'sample_size_per_day']
  thresholds <- c(0.95) 
  ...
  ret <- simulate_poisson(
    ...
    unbalance_ratio = 0.25)
  ret <- ret %>%
    mutate(prob = do.call(vec_prob_poisson, .)) %>%
...
write.csv(pois_design, "output/design_poisson_0.95.csv", row.names = FALSE)
...
#   per_day <- ber_exp_design[row, 'sample_size_per_day']
#   thresholds <- c(0.95)
# 
#   prefix <- paste0(
#     '_ep', effect_p,
...
#     beta2 = beta2)
#   ret <- ret %>%
#     mutate(prob = do.call(vec_prob_bernoulli_exponential, .)) %>%
...
# }
# 
# write.csv(ber_exp_design, "output/design_bernoulli_exponential_0.95.csv", row.names = FALSE)
